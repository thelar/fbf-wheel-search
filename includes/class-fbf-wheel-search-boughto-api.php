    ...
    public function get_wheels($chasis_id, $use_cache = true)
    {
        ...
        if(!empty($transient)&&$this->cache){
            ...
            if(!is_wp_error($response)&&is_array($response)){
                ...
                $data['results'] = $this->simplify($results, ['product_code', 'id', 'seat_type', 'center_bore', 'upstep']); // We only need to store a fraction of the amount of data returned from boughto
                set_transient($key, $data, WEEK_IN_SECONDS);
                return $data;
            }else{
                return $response;
